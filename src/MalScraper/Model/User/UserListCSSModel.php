		...
		if ($content) {
		  ...
		  for ($i = 0; $i < $count; $i++) {
			if (!empty($content[$i]['anime_id'])) {
			  ...
			  $url1 = 'https://shaggyze.website/info/anime/' . $subdirectory . '/' . $content[$i]['anime_id'] . '.json';
			  ...
			  $url1 = 'https://shaggyze.website/info/manga/' . $subdirectory . '/' . $content[$i]['manga_id'] . '.json';
			  if (!filter_var($url2, FILTER_VALIDATE_URL) || !file_get_contents($url2)) {$url2 = $url1;}
			  $content2 = json_decode(file_get_contents(htmlspecialchars_decode($url2)), true);
