...
class UserListCSSModel extends MainModel
{
    ...
    private function _executeMultiCurl(array $handles): array
    {
        ...
        do {
            ...
            if (microtime(true) - $start_batch_time > 5 && $total_handles > 1) {
                echo "\r> [CURL MULTI] Waiting for $running of $total_handles requests to complete...";
        ...
        if ($total_handles > 1) {
            echo "\r> [CURL MULTI] All $total_handles requests completed.   \n";
        }
    ...
    public function getAllInfo()
    {
      ...
      echo "Starting concurrent list page fetching (Batch Size: " . self::LIST_CONCURRENCY_SIZE . ")...\n";
      ...
      while (!$list_finished) {
          ...
          echo "Batch $batch_counter: Requesting offsets $current_offset to " . ($current_offset + ((self::LIST_CONCURRENCY_SIZE - 1) * self::OFFSET_STEP)) . "\n";
      ...
      if ($total_list_entries === 0) {
          echo "Total list entries fetched: 0. Exiting.\n";
          return [];
      }
      echo "Total list entries fetched: $total_list_entries. \n";
      ...
      for ($batch_num = 0; $batch_num < $total_batches; $batch_num++) {
          ...
          // 2. Execute the batch concurrently
          echo "Executing metadata batch " . ($batch_num + 1) . " of $total_batches (" . count($item_handles) . " items)...\n";
