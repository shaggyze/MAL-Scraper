...
class UserCSSModel
{
    ...
    public function getAllInfo()
    {
		...
		if (is_array($list)) {
			...
			foreach ($list as $c) {
				if ($this->_type == 'anime') {
					$temp = str_replace(['{\a}', '{type}', '{status}', '{score}', '{tags}', '{is_rewatching}', '{num_watched_episodes}', '{created_at}', '{updated_at}', '{anime_title}', '{anime_title_eng}', '{synopsis}', '{rank}', '{anime_num_episodes}', '{anime_airing_status}', '{anime_id}'/*, '{anime_studios}', '{anime_licensors}', '{anime_season}'*/, '{anime_total_members}', '{anime_total_scores}', '{anime_score_val}', '{anime_score_diff}', '{anime_popularity}', '{has_episode_video}', '{has_promotion_video}', '{has_video}', '{video_url}'/*, '{genres}'*/, '{title_localized}', '{anime_url}', '{anime_image_path}', '{is_added_to_list}', '{anime_media_type_string}', '{anime_mpaa_rating_string}', '{start_date_string}', '{finish_date_string}', '{anime_start_date_string}', '{anime_end_date_string}', '{days_string}', '{storage_string}', '{priority_string}', '{notes}', '{editable_notes}'], [$c['\a'], 'anime', $c['status'], $c['score'], $c['tags'], $c['is_rewatching'], $c['num_watched_episodes'], $c['created_at'], $c['updated_at'], $c['anime_title'], $c['anime_title_eng'], $c['synopsis'], $c['rank'], $c['anime_num_episodes'], $c['anime_airing_status'], $c['anime_id']/*, $c['anime_studios'], $c['anime_licensors'], $c['anime_season']*/, $c['anime_total_members'], $c['anime_total_scores'], $c['anime_score_val'], $c['anime_score_diff'], $c['anime_popularity'], $c['has_episode_video'], $c['has_promotion_video'], $c['has_video'], $c['video_url']/*, $c['genres']*/, $c['title_localized'], $c['anime_url'], $c['anime_image_path'], $c['is_added_to_list'], $c['anime_media_type_string'], $c['anime_mpaa_rating_string'], $c['start_date_string'], $c['finish_date_string'], $c['anime_start_date_string'], $c['anime_end_date_string'], $c['days_string'], $c['storage_string'], $c['priority_string'], $c['notes'], $c['editable_notes']], $this->_style);
				} else {
